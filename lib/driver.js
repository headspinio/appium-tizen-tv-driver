...
class TizenTVDriver extends BaseDriver {
  ...
  async setupRCApi ({deviceAddress, deviceMac, rcPairingMode}) {
    ...
    if (rcPairingMode) {
      await this.rc.isAvailable();
      await this.rc.getTokenPromise();
    }
  }
  ...
  async startChromedriver ({debuggerPort, executable}) {
    ...
    this.proxyCommand = this.chromedriver.proxyCommand.bind(this.chromedriver);
