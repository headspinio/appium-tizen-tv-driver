...
class TizenTVDriver extends BaseDriver {
  ...
  async setupRCApi ({udid, deviceMac, isDeviceApiSsl}) {
    const localRCPort = await getPortPromise();
    log.info(`Chose local port ${localRCPort} for remote control websocket communication`);
    await forwardPort({udid, remotePort: RC_PORT, localPort: localRCPort});
    this.forwardedPorts.push(localRCPort);
    await this.startRemoteControl({
      port: localRCPort,
      mac: deviceMac,
      secureSocket: isDeviceApiSsl,
    });
  }
  ...
  async startRemoteControl ({port, mac, secureSocket}) {
    this.rc = new RemoteControl({
      debug: true,
      delayCommands: true,
      ip: '127.0.0.1', // should always be local because of port forwarding
      mac,
      port,
      secureSocket,
    });
