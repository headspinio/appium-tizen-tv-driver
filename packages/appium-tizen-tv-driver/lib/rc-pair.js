...
export async function pairRemote({host, port}) {
  ...
  if (deviceData?.device?.TokenAuthSupport === undefined) {
    console.log('The device may not have TokenAuthSupport availability. It does not require remote control token. Please use the remote control with "appium:isDeviceApiSsl" false'); // eslint-disable-line no-console
    return;
  }
