...
- Before running your first session, you should run `appium driver run tizentv pair-remote` to
  ...
  ```
  - Note: The paring popup could appear a couple of times. Please allow all of them.
  - Note: The command could print `Invalid WebSocket frame: invalid status code 1005` error and fail without showing any popups.
    One possible reason here is this paring request was denied in the past. Please check if such a "denied" item exists in the device list below.
